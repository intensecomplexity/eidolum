...
function spawnConfetti(container) {
  if (!container) return;
  const colors = ['#D4A843', '#FDE68A', '#B8860B', '#22c55e', '#0ea5e9', '#f59e0b'];
  for (let i = 0; i < 30; i++) {
...
// 3 steps: welcome → predict → done (removed "how it works")
...
export default function Onboarding({ user, onComplete }) {
  ...
  async function handleSubmitPrediction(e) {
    ...
    setLoading(true);
    try {
      const res = await submitUserPrediction({ ticker, direction, price_target: priceTarget.trim(), evaluation_window_days: windowDays });
      setResult(res);
  ...
  useEffect(() => {
    ...
    if (activeField === 'ticker' && ticker) setTimeout(() => setActiveField('direction'), 400);
  }, [ticker, step]);
  useEffect(() => { if (activeField === 'direction' && direction) setTimeout(() => setActiveField('target'), 400); }, [direction]);
  useEffect(() => { if (activeField === 'target' && priceTarget.trim()) setTimeout(() => setActiveField('timeframe'), 400); }, [priceTarget]);
      ...
      <div className={`w-full max-w-lg transition-opacity duration-200 ${fade ? 'opacity-100' : 'opacity-0'}`}>
        ...
        {STEPS[step] === 'predict' && (
          <div className="py-4">
            ...
            {error && <div className="bg-negative/10 border border-negative/20 rounded-lg px-4 py-2 mb-4 text-sm text-negative text-center">{error}</div>}
            ...
            <form onSubmit={handleSubmitPrediction} className="space-y-4">
              <div className={`card transition-all duration-300 ${pulseClass('ticker')}`}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  {activeField === 'ticker' && <span className="text-[10px] text-accent">Which stock or crypto?</span>}
                </div>
                <TickerSearch value={ticker} onChange={(t, name) => { setTicker(t); setTickerName(name || ''); }} placeholder="TSLA, Bitcoin..." inputClassName="!py-2.5 !text-base" />
              </div>
              ...
              <div className={`card transition-all duration-300 ${pulseClass('direction')}`}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  {activeField === 'direction' && <span className="text-[10px] text-accent">Going up or down?</span>}
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => setDirection('bullish')} className={`flex items-center justify-center gap-2 py-3 rounded-lg border text-sm font-medium transition-colors ${direction === 'bullish' ? 'bg-positive/10 border-positive/40 text-positive' : 'bg-surface-2 border-border text-text-secondary'}`}>
                    <TrendingUp className="w-4 h-4" /> Bullish
                  </button>
                  <button type="button" onClick={() => setDirection('bearish')} className={`flex items-center justify-center gap-2 py-3 rounded-lg border text-sm font-medium transition-colors ${direction === 'bearish' ? 'bg-negative/10 border-negative/40 text-negative' : 'bg-surface-2 border-border text-text-secondary'}`}>
              ...
              <div className={`card transition-all duration-300 ${pulseClass('target')}`}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  {activeField === 'target' && <span className="text-[10px] text-accent">What price will it reach?</span>}
                </div>
              ...
              <div className={`card transition-all duration-300 ${pulseClass('timeframe')}`}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  {activeField === 'timeframe' && <span className="text-[10px] text-accent">When do we check?</span>}
                </div>
        ...
        {STEPS[step] === 'done' && (
          <div className="text-center py-6 relative" ref={confettiRef}>
            ...
            <div className="flex items-center justify-center gap-1.5 text-xs text-muted mb-6">
              <Lock className="w-3 h-3" /> Your prediction is locked. We'll score it automatically when the time comes.
            </div>
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => { handleFinish(); navigate('/leaderboard'); }} className="btn-primary text-sm">Explore the leaderboard</button>
              <button onClick={() => { handleFinish(); navigate('/submit'); }} className="btn-secondary text-sm">Make another call</button>
            </div>
          </div>
        )}
      </div>
