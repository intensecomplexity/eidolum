...
// "online" window. pingPresence() is fire-and-forget, keys anonymous
// visitors by a per-browser localStorage id (multiple tabs = one
// visitor), and reads the auth token fresh on every call, so a
// mid-session login flips the session to authenticated on the next beat
// without a remount.
