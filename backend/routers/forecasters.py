...
# Shared SQL fragments. Two filters now compose:
#   yt_visible       — legacy YouTube rows missing source_timestamp_seconds
#   non_qwen         — Qwen LoRA predictions hidden during audit phase
# Both apply to every user-facing query (leaderboard, profile, ticker,
# activity, stats). Two forms because this file mixes aliased and
# un-aliased queries.
_YT_VIS_BARE = YT_VISIBLE_FILTER_SQL                 # no alias
_YT_VIS_P = yt_visible_filter("p")                   # table alias "p"
...
def _build_accuracy_trend(forecaster_id: int, db: Session, sector: str = None) -> list:
    ...
    try:
        where = (
            ...
            f"AND {_YT_VIS_BARE} AND {_NON_QWEN_BARE}"
        )
...
def get_forecaster(
    ...
    try:
        ...
        raw_sector_rows = db.execute(sql_text(f"""
    ...
    try:
        # Broader exclusion than the original (which only filtered
        # youtube_haiku_v1 + NULL ts): now excludes every YouTube row
        # with NULL source_timestamp_seconds, regardless of classifier.
    ...
    revisions_made = 0
    try:
        revisions_made = int(db.execute(sql_text(f"""
    ...
    try:
        list_rows = db.execute(sql_text(f"""
    ...
    try:
        cat_rows = db.execute(sql_text(f"""
...
def get_forecaster_sectors(request: Request, forecaster_id: int, db: Session = Depends(get_db)):
    ...
    try:
        dir_rows = db.execute(sql_text(f"""
...
def _get_preds(fid, page, limit, filter_type, sector, db):
    ...
    offset = (page - 1) * limit
    # Always hide legacy YouTube rows missing source_timestamp_seconds
    # — they're not countable until the backfill worker fills the
    ...
    where = f" AND {_YT_VIS_P} AND {_NON_QWEN_P}"
