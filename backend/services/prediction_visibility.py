...
def not_excluded_filter(alias: str = "p") -> str:
    """Hide predictions flagged with `excluded_from_training = TRUE`
    from every user-facing surface. Ship #12 originally introduced
    this column to keep the fine-tune loader from seeing bad rows;
    downstream audits (sonnet_rules_v2_mis_attribution, others) reuse
    the same column as the canonical "don't show this to users or
    count it in stats" flag.

    Shape:
        COALESCE(<alias>.excluded_from_training, FALSE) = FALSE

    Rows with NULL (legacy columns) or FALSE pass through; TRUE is
    excluded. Paired with `yt_visible_filter` and `non_qwen_filter`
    on every user-facing query.
    """
    prefix = f"{alias}." if alias else ""
